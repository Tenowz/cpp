...
void	PmergeMe::sortVector(int ac) {
	...
	std::cout << "Time to process a range of " << ac - 1 << " elements " << "with std::vector<unsigned int> : " << time << " µs" << std::endl;
}
...
void	PmergeMe::sortList(int ac) {
	...
	mergeInList(lst);
	double time = static_cast<double>(std::clock() - start) / static_cast<double>(CLOCKS_PER_SEC) * 100000;
	std::cout << "Time to process a range of " << ac - 1 << " elements " << "with std::list<unsigned int> : " << time << " µs" << std::endl;
}